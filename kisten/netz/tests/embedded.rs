...
impl Assets {
    fn new() -> Self {
        ...
        let root = std::env::temp_dir().join(format!(
            "orchester-embedded-{:x}",
            u128::from_ne_bytes(random)
        ));
        std::fs::create_dir_all(&root).unwrap();
        std::fs::write(
            root.join("index.html"),
            "<!doctype html><title>Orchester</title>",
        )
        .unwrap();
        Self(root)
    }
...
async fn authenticate(client: &Client, server: &EmbeddedServer) -> (String, String) {
    ...
    let token = launch
        .split("#fragment_token=")
        .nth(1)
        .expect("launch token");
    let response = client
        .post(format!("{}/api/v1/auth/fragment", server.origin()))
...
async fn embedded_runtime_serves_assets_authenticated_api_and_websocket_then_releases_port() {
    ...
    for path in [
        ...
        let response = client
            .get(format!("{}/api/v1/{path}", server.origin()))
    ...
    let mut request = format!("ws://{address}/api/v1/agents/status/ws")
        .into_client_request()
        .unwrap();
    request
        .headers_mut()
        .insert("origin", server.origin().parse().unwrap());
    request
        .headers_mut()
...
async fn embedded_runtime_rejects_unauthenticated_cross_origin_and_replayed_requests() {
    ...
    assert_eq!(
        client
            .post(format!("{}/api/v1/auth/fragment", server.origin()))
            .header("origin", server.origin())
            .json(&json!({"schema_version": 1, "fragment_token": fragment}))
            .send()
            .await
            .unwrap()
            .status(),
    ...
    for origin in ["https://attacker.example", "null", "http://127.0.0.1:1"] {
        assert_eq!(
            client
                .get(format!("{}/api/v1/agents", server.origin()))
                .header("cookie", &cookie)
...
async fn missing_bundle_assets_fail_startup() {
    let assets = Assets::new();
    assert!(EmbeddedServer::start(
        OrchesterPaths::new(&assets.0, &assets.0),
        assets.0.join("missing")
    )
    .await
    .is_err());
}
